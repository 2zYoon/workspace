...
def check_hp_full():
    global dat
    return dat['stat']['hp'] == dat['stat']['maxhp']

# update hp (delta)
# do not use on battle (why?)
def inc_hp(v):
    global dat
...
def show_ic():
    ...
    if dat['sys']['ic'] == ITEM:
        ...
        # empty item
        if len(item_having) == 0:
            buffer_ro1.set_document(value=Document(txt), bypass_readonly=True)
            buffer_ro8.set_document(value=Document(""), bypass_readonly=True)
            return
        ...
        for j in range(max_cursor_idx+1):
            ...
            if j == ic_cursor:
                ...
                txt += " -> %s [%d]\n" % (ITEM_INFO[item_having[ic_page * WINDOW_IC_HEIGHT + j]][0], dat['stat']['items'][item_having[ic_page * WINDOW_IC_HEIGHT + j]])
            # non-selected
            else:
                txt += "    %s [%d]\n" % (ITEM_INFO[item_having[ic_page * WINDOW_IC_HEIGHT + j]][0], dat['stat']['items'][item_having[ic_page * WINDOW_IC_HEIGHT + j]])
    ...
    # show consumable
        ...
        item_having = []
        for i in range(len(dat['stat']['consumables'])):
            if dat['stat']['consumables'][i] != 0:
                item_having.append(i)
        ...
        for j in range(max_cursor_idx+1):
            ...
            if j == ic_cursor:
                selected = item_having[ic_page * WINDOW_IC_HEIGHT + j]
                txt += " -> %s [%d]\n" % (CONSUMABLE_INFO[item_having[ic_page * WINDOW_IC_HEIGHT + j]][0], dat['stat']['consumables'][item_having[ic_page * WINDOW_IC_HEIGHT + j]])
            # non-selected
            else:
                txt += "    %s [%d]\n" % (CONSUMABLE_INFO[item_having[ic_page * WINDOW_IC_HEIGHT + j]][0], dat['stat']['consumables'][item_having[ic_page * WINDOW_IC_HEIGHT + j]])

        # word wrap
        subtxt = CONSUMABLE_INFO[selected][1]
        nww = len(subtxt) // mid_width
        if nww:
            for j in range(nww+1):
                txt_desc += subtxt[j * mid_width : min((j+1) * mid_width, len(subtxt))]
...
# param: 0 ~ 1.0
# returns 1 when occur, otherwise 0
...
# TODO: choose proper keybind
@kb_main.add('insert')
def kb_main_insert(event):
    global ic_page
    global ic_cursor
    global on_battle
    global game_log
    ...
    if dat['sys']['ic'] == ITEM:
        ...
        for i in range(len(dat['stat']['consumables'])):
            if dat['stat']['consumables'][i] != 0:
                item_having.append(i)
        selected = item_having[ic_page * WINDOW_IC_HEIGHT + ic_cursor]

        # tylenol: recover 50 hp
        if selected == CONSUMABLE_TYLENOL:
            # used only when hp is not full
            if not check_hp_full():
                dat['stat']['consumables'][selected] -= 1
                inc_hp(50)

                if not on_battle:          
                    game_log.append("[INFO] Used tylenol, recovered 50 HP.")
                    show_log()
                else:
                    battle_log.append("> Used tylenol, recovered 50 HP.")
                    show_battle_log()
                show_hp()

        # morphine: recover 350 hp
        elif selected == CONSUMABLE_MORPHINE:
            # used only when hp is not full
            if not check_hp_full():
                dat['stat']['consumables'][selected] -= 1
                inc_hp(350)
                ...
                if not on_battle:          
                    game_log.append("[INFO] Used morphine, recovered 350 HP.")
                    show_log()
                else:
                    battle_log.append("> Used morphine, recovered 350 HP.")
                    show_battle_log()
                show_hp()

        # elixir: get 3 str, 3 int, 3 charm.
        elif selected == CONSUMABLE_ELIXIR:
            if not on_battle:
                dat['stat']['consumables'][selected] -= 1
                ...
                game_log.append("[INFO] Used elixir, got 3 STR, 3 INT and 3 CHARM permanently.")
                update_char_info()
                show_log()

        # soju: get 2 str, lose 1 int
        # (hidden: sometimes lose 3 int by alcoholic)
        elif selected == CONSUMABLE_SOJU:
            if not on_battle:
                if dat['stat']['int'] < 10:
                    game_log.append("[INFO] Required 10 INT to use.")
                else:
                    dat['stat']['consumables'][selected] -= 1

                    game_log.append("[INFO] Used soju, got 2 STR.")
                    dat['stat']['str'] += 2

                    if prob(0.05):
                        dat['stat']['int'] -= 3
                        game_log.append("[INFO] Blacked out! Lost 3 INT permanently.")
                    else:
                        dat['stat']['int'] -= 1
                        game_log.append("[INFO] Lost 1 INT permanently.")
                
                update_char_info()
                show_log()

        # unknown: give random effect
        # super lucky 10%, 
        # positive 35%, 
        # negative 30%, 
        # nothing 20%,
        # super unlucky 5%
        elif selected == CONSUMABLE_UNKNOWN_DRUG:
            if not on_battle:
                if dat['stat']['int'] < 30 or dat['stat']['str'] < 50:
                    game_log.append("[INFO] Required 30 INT & 50 STR to use.")
                else:
                    dat['stat']['consumables'][selected] -= 1
                    p = random.random()
                    ...
                    if p < 0.03:
                        dat['stat']['str'] += 15
                        game_log.append("[INFO] Used unknown drug, got 15 STR.")
                    # get 10 charm (3%) 
                    elif p < 0.06:
                        dat['stat']['charm'] += 10
                        game_log.append("[INFO] Used unknown drug, got 10 CHARM.")
                    # get 15 max HP (2%)
                    elif p < 0.08:
                        dat['stat']['maxhp'] += 15
                        game_log.append("[INFO] Used unknown drug, got 15 max HP.")
                        show_hp()
                    # get 27 int (2%)
                    elif p < 0.10:
                        dat['stat']['int'] += 27
                        game_log.append("[INFO] Used unknown drug, got 27 INT.")
                    # get 2 str (5%)
                    elif p < 0.15:
                        dat['stat']['str'] += 2
                        game_log.append("[INFO] Used unknown drug, got 2 STR.")
                    # get 2 int (5%)
                    elif p < 0.20:
                        dat['stat']['int'] += 2
                        game_log.append("[INFO] Used unknown drug, got 2 INT.")
                    # get 2 charm (5%)
                    elif p < 0.25:
                        dat['stat']['charm'] += 2
                        game_log.append("[INFO] Used unknown drug, got 2 CHARM.")
                    # get 1 stamina (3%)
                    elif p < 0.28:
                        dat['stat']['stamina'] += 1
                        game_log.append("[INFO] Used unknown drug, got 1 STAMINA.")
                    # get 2 luck (5%)
                    elif p < 0.33:
                        dat['stat']['luck'] += 2
                        game_log.append("[INFO] Used unknown drug, got 2 LUCK.")
                    # get 2 max HP (3%)
                    elif p < 0.36:
                        dat['stat']['maxhp'] += 2
                        game_log.append("[INFO] Used unknown drug, got 2 max HP.")
                        show_hp()
                    # get 1 max HP (9%)
                    elif p < 0.45:
                        dat['stat']['maxhp'] += 1
                        game_log.append("[INFO] Used unknown drug, got 1 max HP.")
                        show_hp()
                    # lose 1 int (5%)
                    elif p < 0.50:
                        dat['stat']['int'] -= 1
                        game_log.append("[INFO] Used unknown drug, lost 1 INT.")
                    # lose 2 int (5%)
                    elif p < 0.55:
                        dat['stat']['int'] -= 2
                        game_log.append("[INFO] Used unknown drug, lost 2 INT.")
                    # lose 3 int (5%)
                    elif p < 0.60:
                        dat['stat']['int'] -= 3
                        game_log.append("[INFO] Used unknown drug, lost 3 INT.")
                    # lose 1 str (5%)
                    elif p < 0.65:
                        dat['stat']['str'] -= 1
                        game_log.append("[INFO] Used unknown drug, lost 1 STR.")
                    # lose 2 str (5%)
                    elif p < 0.70:
                        dat['stat']['str'] -= 2
                        game_log.append("[INFO] Used unknown drug, lost 2 STR.")
                    # lose 3 str (5%)
                    elif p < 0.75:
                        dat['stat']['str'] -= 3
                        game_log.append("[INFO] Used unknown drug, lost 3 STR.")
                    # nothing (20%)
                    elif p < 0.95:
                        game_log.append("[INFO] Used unknown drug, nothing happened.")
                    # lose 20 int, 35 str, hp 1
                    else:
                        dat['stat']['str'] -= 35
                        dat['stat']['int'] -= 20
                        dat['stat']['hp'] = 1
                        game_log.append("[INFO] Used unknown drug, something went wrong...")
                        game_log.append("[INFO] Lost 20 INT, 35 STR.")

                update_char_info()
                show_log()
        
        # hongsam: get 1 str
        elif selected == CONSUMABLE_HONGSAM:
            if not on_battle:
                dat['stat']['consumables'][selected] -= 1
                dat['stat']['str'] += 1
                game_log.append("[INFO] Used hongsam, got 1 STR.")

                update_char_info()
                show_log()
        
        # math textbook: get 1 int
        elif selected == CONSUMABLE_MATH_TEXTBOOK:
            if not on_battle:
                dat['stat']['consumables'][selected] -= 1
                dat['stat']['int'] += 1
                game_log.append("[INFO] Used math textbook, got 1 INT.")

                update_char_info()
                show_log()

        # nature: get 4 int
        elif selected == CONSUMABLE_NATURE:
            if not on_battle:
                if dat['stat']['int'] < 70:
                    game_log.append("[INFO] Required 70 INT to use.")
                else:
                    dat['stat']['consumables'][selected] -= 1
                    dat['stat']['int'] += 4
                    game_log.append("[INFO] Used Nature journal, got 4 INT.")

                update_char_info()
                show_log()

        # anavar: random str change
        elif selected == CONSUMABLE_ANAVAR:
            if not on_battle:
                if dat['stat']['str'] < 50:
                    game_log.append("[INFO] Required 50 STR to use.")
                else:
                    dat['stat']['consumables'][selected] -= 1
                    p = random.random()
                    ...
                    if p < 0.01:
                        dat['stat']['str'] += 25
                        game_log.append("[INFO] Used anavar, got 25 STR.")
                    elif p < 0.05:
                        dat['stat']['str'] += 10
                        game_log.append("[INFO] Used anavar, got 10 STR.")
                    elif p < 0.1:
                        dat['stat']['str'] += 7
                        game_log.append("[INFO] Used anavar, got 7 STR.")
                    elif p < 0.5:
                        dat['stat']['str'] += 4
                        game_log.append("[INFO] Used anavar, got 4 STR.")
                    elif p < 0.8:
                        dat['stat']['str'] += 2
                        game_log.append("[INFO] Used anavar, got 2 STR.")
                    elif p < 0.95:
                        dat['stat']['str'] += 1
                        game_log.append("[INFO] Used anavar, got 1 STR.")
                    elif p < 0.99:
                        dat['stat']['str'] -= 4
                        game_log.append("[INFO] Used anavar, lost 4 STR.")
                    elif p < 0.9975:
                        dat['stat']['str'] -= 10
                        game_log.append("[INFO] Used anavar, lost 10 STR.")
                    else:
                        dat['stat']['str'] = 1
                        dat['stat']['hp'] = 1
                        game_log.append("[INFO] Used anavar, big huge side-effect...")
        ...
        # whey: get 2 str
        # (hidden: hp becomes 1, still get 2 str)
        elif selected == CONSUMABLE_WHEY:
            if not on_battle:
                dat['stat']['consumables'][selected] -= 1

                dat['stat']['str'] += 2
                game_log.append("[INFO] Used whey, got 2 STR.")
 
                if prob(0.02):
                    game_log.append("[INFO] F**king diarrhea due to lactose intolerance... lost lots of HP.")
                    dat['stat']['hp'] = 1
            ...
            pass
        #game_log.append("[TEST] ic_page: %d ic_cursor: %d" % (ic_page, ic_cursor))
        #game_log.append("[TEST] selected: %s" % CONSUMABLE_INFO[selected][0])
...
def special_mode():
    ...
    if myname == "RICH":
        ...
        dat['stat']['items'] = [1,2,3,4,5,6,1,2,3]
        dat['stat']['consumables'] = [126,25,53,54,55,75,75,65, 111, 55]
